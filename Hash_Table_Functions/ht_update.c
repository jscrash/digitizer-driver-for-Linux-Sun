/* DEC/CMS REPLACEMENT HISTORY, Element HT_UPDATE.C*/
/* *3    17-AUG-1990 21:55:47 VINCE "(SPR 5644) Code Cleanup"*/
/* *2    21-NOV-1989 10:02:43 CONROY "(SPR 5059) Use subroutine HT_CHECK_FOR_KEY"*/
/* *1    19-JUN-1989 13:01:10 SYSTEM ""*/
/* DEC/CMS REPLACEMENT HISTORY, Element HT_UPDATE.C*/
/*****************************************************************************/
/******************************************************************************
...
Description:
    Update a data element in the hash table.

Prototype:
    publicdef INT ht_update(HASH_TABLE_P hash_table, VOIDPTR key, VOIDPTR data);

Parameters:     (indicate (I)/(O)/(M) for input/output/input-and-modified)
    (I) hash_table  -(HASH_TABLE_P) Pointer to the hash table to be searched.
    (I) key         -(VOIDPTR) Pointer to the desired key.
    (I) data        -(VOIDPTR) Pointer to the area containing the new data.
...
publicdef INT ht_update (HASH_TABLE_P hash_table, VOIDPTR key, VOIDPTR data)
#else
publicdef INT ht_update (hash_table, key, data)
...
VOIDPTR data;
    ...
    if ((hash_table->data_length > 0) && (data == (VOIDPTR)NULL))
        {
    ...
    if ((status = ht_check_for_key (hash_table, key, address, &this_bucket,
        ...
        if (hash_table->data_length != 0)
            ...
            hoblockmove (data, curr_data, hash_table->data_length);
            }
        }
    return status;
    }
