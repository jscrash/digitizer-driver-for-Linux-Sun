...
/* *6    12-AUG-1991 11:03:24 JESSIE "(SPR 0) move harded messages to DB"*/
/* *5     6-AUG-1991 14:25:53 JESSIE "(SPR 8789) assign default value for slope and intercept and add blank checking for those*/
/*items case#3510"*/
...
static INT lg_edit_linear_transform( LOG_EDIT_STRUCT *plg);
static INT lg_edit_ltrans_cdlg_server(INT id, INT item, LOG_EDIT_STRUCT *plg);

#else

static INT lg_edit_linear_transform();
static INT lg_edit_ltrans_cdlg_server();

#endif

/******************************************************************************/
/*                                                                            */
/*    Routine:        LG_EDIT_LTRANS_CDLG_INIT                                */
/*                                                                            */
...
publicdef INT lg_edit_ltrans_cdlg_init( plg)
...
/******************************************************************************/
    ...
    status = am_define_resource(AM_DIALOG, Resource_File,
                                LG_ED_LIN_TRANS, 
                                lg_edit_ltrans_cdlg_server, 
                                &plg->edit_ltrans_cdlg_id,               
                                plg);

                                           /* set up selector control         */

    wisselt( plg->edit_ltrans_cdlg_id, LG_ED_LIN_TRANS_SELECTOR,
             LG_ED_LIN_TRANS_VSCROLL, plg->num_traces,
             1, plg->trace_tcp);

                                           /* set default selec. to 1st item  */
    wi_set_default_sel( plg->edit_ltrans_cdlg_id,
                        LG_ED_LIN_TRANS_SELECTOR, 1);

    /********************************************************************/
    /* set default value for slope and intercept                        */
    /********************************************************************/
    strcpy(value,"1.0");
    wi_set_dialog_text (plg->edit_ltrans_cdlg_id, LG_ED_LIN_TRANS_SLOPE, value);
    strcpy(value,"0.0");
    wi_set_dialog_text (plg->edit_ltrans_cdlg_id, LG_ED_LIN_TRANS_INTERCEPT, 
			value);

                                           /* bring up dialog box             */
    status = wiopcd(plg->edit_ltrans_cdlg_id); 
...
static INT lg_edit_ltrans_cdlg_server( id, item, plg)
...
/******************************************************************************/
  ...
  switch (item)
    {   
      case LG_ED_LIN_TRANS_CANCEL:        /* Cancel button pressed, exit     */
           status = SUCCESS;
           break;
      case LG_ED_LIN_TRANS_EXECUTE:       /* Branch to copy routine          */
           status = lg_edit_linear_transform( plg);
           break;
    ...
    if (status == SUCCESS )
      {
       status = am_release_resource( plg->edit_ltrans_cdlg_id);
...
static INT lg_edit_linear_transform( plg)
...
/******************************************************************************/
  ...
  wiqsel( plg->edit_ltrans_cdlg_id, LG_ED_LIN_TRANS_SELECTOR,
  ...
  wi_query_dialog_item( plg->edit_ltrans_cdlg_id, LG_ED_LIN_TRANS_TRACE,
                        &value, text );
  ts_noblanks(text);
  if ( strlen(text) == 0 )
    {
  ...
  wi_query_dialog_item( plg->edit_ltrans_cdlg_id, LG_ED_LIN_TRANS_SERVICE,
                        &value, text );
  ts_noblanks(text);
  ...
  wi_query_dialog_item( plg->edit_ltrans_cdlg_id, LG_ED_LIN_TRANS_UNITS,
                        &value, text );
  ...
  /*******************************************************************/
  wi_query_dialog_item( plg->edit_ltrans_cdlg_id, LG_ED_LIN_TRANS_SLOPE,
                        &value, text );
  if (strlen(ts_noblanks(text)) == 0)
    {
    wi_beep(1);
    /*am_message (AM_ALERT, "Please enter Slope(m) value.");*/
    /* move message to Data Base */
    am_message (AM_ALERT, mg_message(LG_ED_SLOPE_ENTER));
    return (FAIL);     
    }
  else
    {
    sscanf( text, "%lf", &slope);    /* slope is double */
    if (slope == 0.0)
      {
       /*am_message( AM_ALERT, "Slope cannot be = 0.0");*/
       /* move message to Data Base */
       am_message (AM_ALERT, mg_message(LG_ED_INVALID_SLOPE));
       return (FAIL);     
      }

    wi_query_dialog_item( plg->edit_ltrans_cdlg_id, LG_ED_LIN_TRANS_INTERCEPT,
                        &value, text );
    if (strlen(ts_noblanks(text)) == 0)
      {
      wi_beep(1);
      /*am_message (AM_ALERT, "Please enter Intercept(b) value.");*/
      /* move message to Data Base */
      am_message (AM_ALERT, mg_message(LG_ED_INTERCEPT_ENTER));
      return (FAIL);     
      }
    else
      {
      sscanf( text, "%lf", &intercept);
      }
    }
...
/* ---------------------------COMMENTED OUT!!!---------------------------
  if (slope == 0.0 && intercept == 0.0)
    {
     am_message( AM_ALERT, "Slope and intercept cannot both be = 0.0");
     return (FAIL);     
    }
---------------------------------TO HERE!!!------------------------------ */
