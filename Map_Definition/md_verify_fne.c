/* DEC/CMS REPLACEMENT HISTORY, Element MD_VERIFY_FNE.C*/
/* *2    17-AUG-1990 21:58:28 VINCE "(SPR 5644) Code Cleanup"*/
/* *1    19-JUN-1989 13:07:26 SYSTEM ""*/
/* DEC/CMS REPLACEMENT HISTORY, Element MD_VERIFY_FNE.C*/
/*****************************************************************************/
/******************************************************************************
...
Prototype:
    publicdef INT md_verify_fne(MD_KEY_INFO *key_info,CHAR parm_string[MD_MAX_PARM_STRING],
...
/* BEGIN:   */
#if USE_PROTOTYPES
publicdef INT md_verify_fne (MD_KEY_INFO *key_info,
                             CHAR parm_string[MD_MAX_PARM_STRING],
                             MD_PARM_STRUCT **parmstruc)
#else
publicdef INT md_verify_fne (key_info, parm_string, parmstruc)
...
MD_PARM_STRUCT **parmstruc;
    ...
    if (strlen (parm_string) >= sizeof(FILENAME))
        {
        status = MD_BAD_FILENAME;
        }
    else
        {
        if (strlen (key_info->val_arg) != 0)
            {
            ho_add_path (key_info->val_arg, parm_string, sizeof(FILENAME), filename);
            }
        else
            {
            strcpy (filename, parm_string);
            }
        if (hoexists (filename))
            {
