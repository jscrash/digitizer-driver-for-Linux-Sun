/* DEC/CMS REPLACEMENT HISTORY, Element MD_VER_TLIST.C*/
/* *2    17-AUG-1990 21:59:32 VINCE "(SPR 5644) Code Cleanup"*/
/* *1    19-JUN-1989 13:08:11 SYSTEM ""*/
/* DEC/CMS REPLACEMENT HISTORY, Element MD_VER_TLIST.C*/
/*****************************************************************************/
/******************************************************************************
...
Description:
    Function to validate a TLIST parameter value.

Prototype:
    publicdef INT md_verify_tlist(MD_KEY_INFO *key_info,CHAR parm_string[MD_MAX_PARM_STRING],
...
/* BEGIN:   */
#if USE_PROTOTYPES
publicdef INT md_verify_tlist (MD_KEY_INFO *key_info,
                               CHAR parm_string[MD_MAX_PARM_STRING],
                               MD_PARM_STRUCT **parmstruc)
#else
publicdef INT md_verify_tlist (key_info, parm_string, parmstruc)
...
MD_PARM_STRUCT **parmstruc;
    ...
    if (plen > 0)
        ...
        status = md_verify_tlu (key_info, temp_string, &temp_parm1);
        if (status EQUALS SUCCESS)
            {
            if (cptr != NULL)
                {
                status = md_verify_tlist (key_info, ++cptr, &temp_parm2);
