/* DEC/CMS REPLACEMENT HISTORY, Element LD_MIMIC_EXP_SERVER.C */
/* *3    14-AUG-1991 11:59:24 KEE "(SPR -1) Put in File Lock/Unlock logic" */
/* *2     1-MAY-1991 22:10:10 KEE "(SPR -1) Mimic Export" */
/* *1     1-MAY-1991 21:27:43 KEE "Landmark Import & Export, Zycor Export and Mimic Export" */
/* DEC/CMS REPLACEMENT HISTORY, Element LD_MIMIC_EXP_SERVER.C */
/******************************************************************************     */
/*                                                                                  */
...
/* TITLE : LD_MIMIC_EXP_SERVER - main server for export to MIMIC*/
...
#if USE_PROTOTYPES
static INT ld_mimic_exp_batch_job (MIMIC_WORKSPACE *workspace);
#else
static INT  ld_mimic_exp_batch_job();
#endif

/* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   **
   **   ROUTINE:  ld_mimic_exp_server (id, item, pointer)
   **
   **   DESCRIPTION:
   **
...
*/
#if USE_PROTOTYPES
publicdef INT ld_mimic_exp_server(INT id, INT item, MIMIC_WORKSPACE *workspace)
#else
publicdef INT ld_mimic_exp_server(id, item, workspace)
INT                 id;
INT                 item;
...
#endif
{
	...
	case MEX_OKAY:
	    ...
	    goFlag = TRUE;
	    wi_query_selections(workspace->dialogID, MEX_WHICH_MAP,
					&item_count, items);
	    if (item_count == 0)
		{
		wi_beep(1);
	    ...
	    if (goFlag == TRUE)
		{
		status = wi_query_dialog_item(workspace->dialogID, MEX_FAULT, 
					&faultVal, text);
		status = wi_query_dialog_item(workspace->dialogID, MEX_XYZ,
					&xyzVal, text);
		if (faultVal == 0 && xyzVal == 0)
		    {
		    wi_beep(1);
		    wi_alert(0, "", "",
	    ...
	    if (goFlag)
		{
	        ld_mimic_file_pipe(workspace, items[0], faultVal, xyzVal, &goFlag);
		}
	    if (goFlag)
		{
	     	ld_mimic_exp_batch_job(workspace);
	     	am_quit();
		}
	     break;
...
/* %%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
   **
   **   ROUTINE:  ld_mimic_exp_batch_job(workspace)
   **
   **   DESCRIPTION:
   **
...
static INT ld_mimic_exp_batch_job (workspace)
MIMIC_WORKSPACE     *workspace;
{
     ...
     PROGNAME ("ld_mimic_exp_batch_job") ;
...
/*   name of the batch loader   */
     ho_add_path("ESI$BIN", "ld_mimic_batch", 
...
/*  name of the DCL file which is submitted to run the loader  */
     ...
     sprintf(comi_file, "MIE_%s.COM", baseFilename);

#endif
#if UNIX
/*    name of the script file which is submitted to run the loader  */
     sprintf(comi_file, "mimicexp%s", date_tag);
...
/*  name of the CPL file which is submitted to run the loader  */
     sprintf(comi_file, "MIE_%s.CPL", date_tag);
     ...
     if (value == TRUE)
	{
     ...
     status = wi_query_selections(workspace->dialogID, MEX_WHICH_MAP,
					&item_count, items);
	...
	fprintf(comi_fp, "MAPNAME:%s\n", 
     ...
     if (code)
	{
	sprintf(message, "Export to MIMIC, can't spawn %s, code %d",
			comi_file, code);
	}
     else
	{
	sprintf(message, "Export to MIMIC being started as job #%d", 
				job_user_no);
	}
