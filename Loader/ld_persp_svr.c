/* DEC/CMS REPLACEMENT HISTORY, Element LD_PERSP_SVR.C*/
/* *5    14-AUG-1991 11:59:38 KEE "(SPR -1) Put in File Lock/Unlock logic"*/
/* *4    14-AUG-1990 13:32:26 VINCE "(SPR 5644) Header Standardization"*/
/* *3     6-FEB-1990 10:56:54 MING "(SPR 0) change date/time declaration to new data type DATE_TIMRE"*/
/* *2    26-JUN-1989 10:47:25 GILLESPIE "(SPR 20) Fix castings"*/
/* *1    19-JUN-1989 13:03:12 SYSTEM ""*/
/* DEC/CMS REPLACEMENT HISTORY, Element LD_PERSP_SVR.C*/
/*****************************************************************************/
/******************************************************************************
...
Description:
    This function services events from the perspective file loader
    custom dialog.
    
Prototype:
    publicdef INT ld_perspective_srvr(INT id, INT item, BYTE *p);
...
publicdef INT ld_perspective_srvr (INT id, INT item, BYTE *p) 
#else
publicdef INT ld_perspective_srvr (id, item, p)
INT id;
INT item;
BYTE *p;
#endif
...
/* ----------------- */
    ...
    num_files = ts_tcp_len (sgc_files->perspective.file_list);
    if (num_files <= 0)
        {
        return SUCCESS;
        }
    ...
    wi_query_selections (id, LD_PERSPECTIVE_SELECT, &num_files, select_list);
    if (num_files > 0)
        {
        ...
        if (item EQUALS LD_RUN_4)
            {
            ldz_load_nlist_files (4, &(sgc_files->perspective), select_list,
                                  num_files);
            }
        /* * SUBMIT LOADING TO A BATCH PROCESS * */
        /* ------------------------------------- */
        
        else if (item EQUALS LD_SUBMIT_4)
            {
            ...
            sprintf (temp_filename, "ld_perspective_%s.data", date_time);
	    ...
	    if (status != SUCCESS)
	      return status;
            ...
            fprintf (temp_file, "3");   /* DATA TYPE */
            fprintf (temp_file, "\n%s", sgc_files->perspective.directory);
            fprintf (temp_file, "\n%d", num_files);
            for (i = 0; i < num_files; i++)
                {
                fprintf (temp_file, "\n%s",
                         sgc_files->perspective.file_list[select_list[i] - 1]);
                }
