/* DEC/CMS REPLACEMENT HISTORY, Element LD_CON_SVR.C*/
/* *5    14-AUG-1991 11:57:32 KEE "(SPR -1) Put in File Lock/Unlock logic"*/
/* *4    14-AUG-1990 13:31:29 VINCE "(SPR 5644) Header Standardization"*/
/* *3     6-FEB-1990 10:50:51 MING "(SPR 0) change date/time declaration to new data type DATE_TIME"*/
/* *2    26-JUN-1989 10:46:26 GILLESPIE "(SPR 20) Fix castings"*/
/* *1    19-JUN-1989 13:02:51 SYSTEM ""*/
/* DEC/CMS REPLACEMENT HISTORY, Element LD_CON_SVR.C*/
/*****************************************************************************/
/******************************************************************************
...
Description:
    Service events from the contour file loader custom dialog.

Prototype:
    publicdef INT ld_contour_srvr(INT id, INT item, BYTE *p);
...
publicdef INT ld_contour_srvr (INT id, INT item, BYTE *p) 
#else
publicdef INT ld_contour_srvr (id, item, p)
INT id;
INT item;
BYTE *p;
#endif
...
/* ----------------- */
    ...
    num_files = ts_tcp_len (sgc_files->contour.file_list);
    if (num_files <= 0)
        {
        return SUCCESS;
        }
    ...
    wi_query_selections (id, LD_CONTOUR_SELECT, &num_files, select_list);
    if (num_files > 0)
        {
        ...
        if (item EQUALS LD_RUN_3)
            {
            ldz_load_nlist_files (3, &(sgc_files->contour), select_list, num_files);
            }
        /* * SUBMIT LOADING TO A BATCH PROCESS * */
        /* ------------------------------------- */
        
        else if (item EQUALS LD_SUBMIT_3)
            {
            ...
            sprintf (temp_filename, "ld_contour_%s.data", date_time);
	    ...
	    if (status != SUCCESS)
	      return status;
            ...
            fprintf (temp_file, "\n%s", sgc_files->contour.directory);
            fprintf (temp_file, "\n%d", num_files);
            for (i = 0; i < num_files; i++)
                {
                fprintf (temp_file, "\n%s",
                         sgc_files->contour.file_list[select_list[i] - 1]);
                }
