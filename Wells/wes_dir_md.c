/* DEC/CMS REPLACEMENT HISTORY, Element WES_DIR_MD.C */
/* *2     1-MAY-1991 22:51:18 KEE "(SPR -1) Landmark Import & Export" */
/* *1     1-MAY-1991 21:34:49 KEE "Landmark Import & Export, Zycor Export and Mimic Export" */
/* DEC/CMS REPLACEMENT HISTORY, Element WES_DIR_MD.C */
/*****************************************************************************/
/******************************************************************************
...
Description:
    ...
    3) For non-deviated wells, return dx = dy = 0 and md = tvd.
...
Prototype:
    publicdef INT wes_get_dir_md(UWI uwi, DOUBLE in_md, DOUBLE *dx, DOUBLE *dy, 
    DOUBLE *tvd);
...
/* BEGIN:   */
#if USE_PROTOTYPES
publicdef INT wes_get_dir_md (UWI uwi, DOUBLE in_tvd, DOUBLE *dx, DOUBLE *dy,
                               DOUBLE *md) 
#else
publicdef INT wes_get_dir_md (uwi, in_tvd, dx, dy, md)
UWI uwi;
DOUBLE in_tvd;
DOUBLE *dx;
DOUBLE *dy;
DOUBLE *md;
    ...
    case 'Y':
        ...
        if (pWell->deviation.nlist EQUALS (NLIST_HEADER)0)
            ...
            status = wes_load_dir_nlist (uwi, &(pWell->deviation.nlist));
            ...
            status = nl_set_current_row (pWell->deviation.nlist, 1, 1);
            }
        ...
        status = nl_get_rows (nlist, 1, ndim, Dim_List, Value_List, &npoints);
        
        /*  back up until you find a measured depth <=  */
        /*  the desired measured depth          */
        
        while (status >= 0 AND NOT (status & NL_START_OF_STROKE) AND current.tvd > in_tvd)
            {
            status = nl_previous_row (nlist, ndim, Dim_List, Value_List);
            }
        if (status >= 0 AND (current.tvd > in_tvd))
            {
            ...
            ndim = 4;
            nl_get_rows (nlist, 1, ndim, Dim_List, Value_List, &npoints);
            tc_zeroes (&above, sizeof above);
            status = WE_INCOMPLETE_SURVEY;
            break;
            }
        ...
        for (status = 0;
             status >= 0 AND NOT (status & NL_END_OF_STROKE) AND current.tvd < in_tvd; )
            {
            above = current;
            status = nl_next_row (nlist, ndim, Dim_List, Value_List);
            }
        if (status >= 0 AND (current.tvd < in_tvd))
            {
            /*  this case is reached when the deviation
        ...
        ndim = 4;
        nl_previous_row (nlist, ndim, Dim_List, Value_List);
        above = current;
        nl_next_row (nlist, ndim, Dim_List, Value_List);
        break;
    ...
    if (current.tvd EQUALS above.tvd)
        {
        *dx = current.dx;
        *dy = current.dy;
        *md = current.md;
        ...
        {
        frac = (current.tvd - in_tvd) / (current.tvd - above.tvd);
        *dx = current.dx - frac * (current.dx - above.dx);
        *dy = current.dy - frac * (current.dy - above.dy);
        *md = current.md - frac * (current.md - above.md);
        }
    if (pWell->deviation.nlist != (NLIST_HEADER) 0)
       {
         nl_free_nlist(pWell->deviation.nlist);
         pWell->deviation.nlist = (NLIST_HEADER) 0;
       }
    return status;
    }
/* END:     */
