/* DEC/CMS REPLACEMENT HISTORY, Element NL_ADD_ROWSC.C*/
/* *4    25-OCT-1990 12:20:46 GILLESPIE "(SPR 31) Lint run - correct casting inconsistencies"*/
/* *3    25-JUL-1990 17:35:29 VINCE "(SPR 5644) Fixed compile errors introduced with the header changes"*/
/* *2    25-JUL-1990 14:55:45 VINCE "(SPR 5644) Header Standardization"*/
/* *1     5-MAR-1990 10:57:26 GILLESPIE "Rename files for Unix compliance (SPR 6015)"*/
/* DEC/CMS REPLACEMENT HISTORY, Element NL_ADD_ROWSC.C*/
/* DEC/CMS REPLACEMENT HISTORY, Element NL_ADD_PTSCNV.C*/
/* *1    19-JUN-1989 13:17:44 SYSTEM ""*/
/* DEC/CMS REPLACEMENT HISTORY, Element NL_ADD_PTSCNV.C*/
/*****************************************************************************/
/******************************************************************************
...
Description:
    Add one or more data rows to the current table in an n-List
    and perform any required data type conversion.

Prototype:
    publicdef INT nl_add_rows_convert(NLIST_HEADER nlist, UINT nrows, UINT nCol, 
        UINT *Col_List, VOIDPTR *Var_List, INT *Data_Units, INT *Data_Types, 
        size_t *Data_Widths);
...
Parameters:     (indicate (I)/(O)/(M) for input/output/input-and-modified)
    ...
    (I) Var_List    -(VOIDPTR *) Array of addresses of start of data for each
                        column.
    Data_Units      -(INT *) Save for the future.         
    ...
    Data_Widths     -(size_t *) Array of data widths represented by input data.

Return Value/Status:
    Function returns the status of nlz_aid_rows_convert.
...
#else
publicdef INT nl_add_rows_convert (nlist, nrows, nCol, Col_List, Var_List, Data_Units,
                                   Data_Widths, Data_Types)
NLIST_HEADER nlist;
UINT nrows;
UINT nCol;
UINT *Col_List;
