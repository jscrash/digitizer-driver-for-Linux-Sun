...
/* *6    25-OCT-1990 12:22:18 GILLESPIE "(SPR 31) Lint run - correct casting inconsistencies"*/
...
Prototype:
    publicdef INT nl_draw_table (NLIST_HEADER nlist, UINT table_num, UINT independent, 
        UINT dependent);
...
publicdef INT nl_draw_table (NLIST_HEADER nlist, UINT table_num, UINT independent,
                             UINT dependent)
#else
publicdef INT nl_draw_table (nlist, table_num, independent, dependent)
NLIST_HEADER nlist;
UINT table_num;
UINT independent;
UINT dependent;
    ...
    for (i = 0, x = pidata, y = pddata; i < ptable->nrows;
            ...
            for (j = 0, sub_nrows = 0; j < nrows; j++, sub_nrows++)
                ...
                if (((inull < 0 && x[j] >= inull * 1.01 && x[j] <= inull * 0.99) ||
                    ...
                    ((dnull < 0 && y[j] >= dnull * 1.01 && y[j] <= dnull * 0.99) ||
                     (dnull > 0 && y[j] <= dnull * 1.01 && y[j] >= dnull * 0.99)))
                    {
                    if (sub_nrows > 1)  /* only draw more than two rows.  */
                        {
                        xptr = &x[j - sub_nrows];
                        yptr = &y[j - sub_nrows];
