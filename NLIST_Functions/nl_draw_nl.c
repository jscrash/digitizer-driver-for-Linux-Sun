...
/* *3    18-MAR-1991 17:10:29 JULIAN "(SPR 0) Lease Mapping init checkin"*/
...
publicdef INT nl_draw_nlist (NLIST_HEADER nlist, UINT independent, UINT dependent)
#else
publicdef INT nl_draw_nlist (nlist, independent, dependent)
NLIST_HEADER nlist;
UINT independent;
UINT dependent;
#endif
    ...
    for (i = 0; i < nlist->total_tables; i++)
	    ...
	    /* these "if statement" is added for the use of lease overlay
	       complex polyline drawing. Since nl_draw_nlist will not draw it
	       correctly, so temporarily we just don't draw any complex case
	       polyline until we find a way to draw them properly
	       (purna - (10/16/90) */
	if(nlist->current_table_ptr != (TABLE_HEADER *)0)
	   {
	   if((nlist->current_table_ptr->name EQUALS (CHAR *)0) OR 
	       ARE_DIFFERENT(nlist->current_table_ptr->name,"COMPLEX"))        
	       nl_draw_table (nlist, i + 1, independent, dependent);
           }
    return SUCCESS;
