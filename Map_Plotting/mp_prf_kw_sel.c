/*  DEC/CMS REPLACEMENT HISTORY, Element MP_PRF_KW_SEL.C*/
/*  *3    17-AUG-1990 22:08:02 VINCE "(SPR 5644) Code Cleanup"*/
/*  *2    26-JUN-1989 10:58:43 GILLESPIE "(SPR 20) Fix castings"*/
/*  *1    19-JUN-1989 13:14:05 SYSTEM ""*/
/*  DEC/CMS REPLACEMENT HISTORY, Element MP_PRF_KW_SEL.C*/
/*****************************************************************************/
/******************************************************************************
...
Description:
    ...
    preference keyword has been picked.  This includes setting up
    the options selector box and updating the state of the dialog.

Prototype:
    publicdef INT mp_pref_keyword_selected(INT id);
...
publicdef INT mp_pref_keyword_selected (INT id)
#else
publicdef INT mp_pref_keyword_selected (id)
INT id;
#endif
    {
    ...
    /* * CHECK THE SELECTED KEYWORD IS NEITHER '---' NOR BLANK * */
    /* --------------------------------------------------------- */
    
    wi_set_default_sel (id, MD_PARM_PREFS, 0);
    wi_query_selections (id, MD_KEY_PREFS, &i, &j);
    ...
    if (display->key_ndx[i] EQUALS - 1)
        {
        wi_beep (1);
        sprintf (msg, "%s: '%s'", mg_message (MD_INVALID_KEY), display->keywords[i]);
    ...
    /* --------------------------- */
        ...
        md_state->button_status[5] = (temp_key_info->parm_req) ? ON : OFF;
        
        if (display->verb_ndx[i] != md_state->active_verb_ndx)
            {
