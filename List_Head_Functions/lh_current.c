/* DEC/CMS REPLACEMENT HISTORY, Element LH_CURRENT.C*/
/* *1    19-JUN-1989 13:05:42 SYSTEM ""*/
/* DEC/CMS REPLACEMENT HISTORY, Element LH_CURRENT.C*/
...
INT lh_current(header_pointer, key_pointer, data_pointer, data_length) 
/************************* lh_current *********************************
...
-DESCRIPTION:
  ...
  This routine returns the key and data of the current node; i.e., the
  node referenced by header_pointer->CURRENT_NODE.  If that field is 
  NULL, then the first list element is made current, and that node is 
  returned.

  Note that, if either of the key_pointer or data_pointer areas is set
  to NULL, then that copy is not performed.
...
/*
    Copy the data areas.
*/
    ptr = (lh_listnode *)((header_pointer->CURRENT_NODE==(lh_listnode *)NULL) ?
