/* DEC/CMS REPLACEMENT HISTORY, Element CT_PRJ_NL_XY.C*/
/* *3    14-AUG-1990 13:07:11 VINCE "(SPR 5644) Header Standardization"*/
/* *2    30-JUN-1989 10:35:37 GILLESPIE "(SPR 33) Fix INT/UINT typedefs in calling sequence"*/
/* *1    19-JUN-1989 12:45:22 SYSTEM ""*/
/* DEC/CMS REPLACEMENT HISTORY, Element CT_PRJ_NL_XY.C*/
/*****************************************************************************/
/******************************************************************************
...
Description:
    This function converts an nlist from the map to the project projection.
    
Prototype:
    publicdef INT ct_project_nlist_xy(NLIST_HEADER nlist_in, NLIST_HEADER *nlist_out,
...
Return Value/Status:
    ...
    CT_SAME_PROJECTIONS - Error if map and project projections are the same.
...
publicdef INT ct_project_nlist_xy (NLIST_HEADER nlist_in, NLIST_HEADER *nlist_out,
                                   UINT *dim_list, DOUBLE tolerance) 
#else
publicdef INT ct_project_nlist_xy (nlist_in, nlist_out, dim_list, tolerance)
...
DOUBLE tolerance;
    ...
    if (strcmp (project_projection, map_projection) EQUALS ZERO)
        ...
        status = ct_transform_nlist (nlist_in, map_data, nlist_out, project_data,
                                     dim_list, tolerance);
        }
    return status;
    }
